...
const FlashcardModal: React.FC<FlashcardModalProps> = ({ element, onClose }) => {
  ...
  return (
    ...
    >
      {/* Entire modal background with element color gradient */}
      <div 
        className={`relative bg-gradient-to-br ${styles.bg} rounded-xl max-w-lg w-full max-h-[85vh] overflow-hidden shadow-2xl`}
      >
        ...
        >
          <div className="relative bg-black bg-opacity-40 -m-6 p-6">
        ...
        {/* Content with semi-transparent white background for readability */}
        <div className="p-6 space-y-4 overflow-y-auto max-h-96 bg-white bg-opacity-90">
        ...
        {/* Footer with element-colored background */}
        <div className="p-6 pt-0 bg-white bg-opacity-90">
          <button
            type="button"
            onClick={handleClose}
            className="bg-red-700 hover:bg-red-600 w-full py-3 px-6 rounded-lg font-semibold text-white text-lg transition-all duration-200 shadow-md hover:shadow-lg active:scale-95"
          >
            CONTINUE
          </button>
        </div>
